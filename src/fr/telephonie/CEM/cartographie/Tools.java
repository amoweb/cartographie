...
public class Tools {
	...
	public static boolean generateKML(List<Point> points, String file) throws IOException
	{
		...
		for(Point p : points)
		{
			...
			bw.write("		<Placemark>\r\n"
					+ "			<visibility>0</visibility>\r\n"
					+ "			<name>" + i + "</name>\r\n"
					+ "			<description>"+p.getValue()+ " " + p.getUnite() + "</description>\r\n"
	...
	{
		long deltaLoc = loc.get(2).getTimestamp() - loc.get(1).getTimestamp();
		long deltaVal = val.get(2).getTimestamp() - val.get(1).getTimestamp();
	...
	public static String filetype(String dataFile) throws IOException
	{
		...
		else if(dataFile.toLowerCase().endsWith(".csv"))
		{
			...
			else if(firstLine.startsWith("About;"))
			{
				return "envionic";
			}
			else
	...
	/**
	 * Charge le contenu du fichier Envionic
	 * @param dataFile
	 * @param day (timestamp)
	 ...
	 * @throws ParseException 
	 */
	public static List<Point> loadEnvionic(String dataFile, long day) throws FileNotFoundException, IOException, ParseException {
		List<Point> l = new LinkedList<Point>();
		...
		// Elec;16:12:56;start;;unit;max;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
		Pattern p = Pattern.compile("^[^;]*;([^;]*);[^;]*;[^;]*;([^;]*);([^;]*);.*");
		while(ligne != null)
		{
			if(ligne.startsWith("Elec;") && ligne.contains("log"))
			{
				Matcher m = p.matcher(ligne);
				if(m.matches())
				{
					...
					point.setUnite(unite);
					
					long hoursMiliSeconds = (Long.parseLong(hours.substring(0,2))*60*60
						+ Long.parseLong(hours.substring(3,5))*60
						+ Long.parseLong(hours.substring(6,8)))
						* 1000;
					
					point.setTimestamp(day+hoursMiliSeconds);
					
					point.setValue(Double.parseDouble(val.replace(',', '.')));
					l.add(point);
				}
			}
