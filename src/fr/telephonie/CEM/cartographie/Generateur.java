...
public class Generateur
{
	 ...
	 */
	private List<Point> preparePoints(List<Point> lpos, List<Point> lval)
		throws NoEnoughtPointsException, CorrelationErrorException,
		NumberFormatException, MathParsingException, NullAltitudeException
	{	
		...
		{
			if(!finale.get(i).getLongitude().isEmpty() && !finale.get(i).getLatitude().isEmpty())
				this.lfinale.add(finale.get(i));
			else
				System.out.println("Invalid point : " + finale.get(i));
		}
	 ...
	 * @param fichier de donnée (Aaronia, Envionic ou Lascar)
	 * @throws ParseException 
	 * @throws IOException 
	 * @throws NumberFormatException 
	 * @throws CorrelationErrorException 
	...
	NoEnoughtPointsException, NullAltitudeException, MathParsingException, LocationFormatException
	{
		...
		else if(Tools.filetype(dataFile).contentEquals("envionic"))
		{
			// Les fichiers Envionic ne contiennent pas la date du jour. Nous prennons donc celle du fichier GPX
			long timestampDay00h00 = ((this.lpos.get(0).getTimestamp()/1000) - (this.lpos.get(0).getTimestamp()/1000)%(24*60*60))*1000;
			lfinale = this.preparePoints(this.lpos, Tools.loadEnvionic(dataFile, timestampDay00h00));
		}
		else
		{
	 ...
	 * @throws EvaluationCheckException 
	 */
	public void generateKMLfromData(String dataFile, String mediumValue, String highValue) throws NumberFormatException, NoEnoughtPointsException, CorrelationErrorException, MathParsingException, NullAltitudeException, IOException, ParseException, LocationFormatException
	{
		List<Point> lfinale = null;	
		if(Tools.filetype(dataFile).contentEquals("narda"))
		{
		...
		else if(Tools.filetype(dataFile).contentEquals("envionic"))
		{
			// Les fichiers Envionic ne contiennent pas la date du jour. Nous prennons donc celle du fichier GPX
			long timestampDay00h00 = ((this.lpos.get(0).getTimestamp()/1000) - (this.lpos.get(0).getTimestamp()/1000)%(24*60*60))*1000;
			lfinale = this.preparePoints(Tools.loadEnvionic(dataFile, timestampDay00h00), null);
		}
		else
		{
			lfinale = this.preparePoints(this.lpos, Tools.loadData(dataFile));	
